...
- (void)layoutFatherContainers
{
    ...
    if (self.superNavItemNode == nil) { //父容器为nil 表示为maincontainer
        self.rootView.frame = CGRectMake(0, GStatusBarHeight, G_SCREEN_WIDTH, G_NAV_HEIGHT -GStatusBarHeight);
...
- (void)layoutLeftSubContainers:(GNavigationItem*)headerItem
{
    ...
    CGRect headerRect = CGRectMake(left, (GNavBarContentHeight - size.height)*0.5, size.width, size.height);
...
- (void)layoutRightSubContainers:(GNavigationItem*)headerItem
{
    ...
    CGRect headerRect = CGRectMake(right - size.width, (GNavBarContentHeight - size.height)*0.5, size.width, size.height);
    headerItem.rootView.frame = headerRect;
